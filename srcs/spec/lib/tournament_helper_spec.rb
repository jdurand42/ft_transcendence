...
RSpec.describe TournamentHelper do
  context 'Tournament with 3 participants' do
    ...
    describe 'match_all_played?' do
      it 'returns true' do
        create(:game, mode: 'tournament', status: 'played', tournament_id: tournament.id,
                      player_left_id: participants[0], player_right_id: participants[1], winner_id: participants[0])
        create(:game, mode: 'tournament', status: 'played', tournament_id: tournament.id,
                      player_left_id: participants[1], player_right_id: participants[2], winner_id: participants[1])
        create(:game, mode: 'tournament', status: 'played', tournament_id: tournament.id,
                      player_left_id: participants[2], player_right_id: participants[0], winner_id: participants[2])
      ...
      it "returns false if all game status not 'played'" do
        ...
        create(:game, mode: 'tournament', status: 'played', tournament_id: tournament.id,
                      player_left_id: participants[1], player_right_id: participants[2], winner_id: participants[1])
        create(:game, mode: 'tournament', status: 'inprogress', tournament_id: tournament.id,
                      player_left_id: participants[2], player_right_id: participants[0])
        expect(match_all_played?).to be_falsey
      end
    end
    describe 'participant win_count' do
      it 'increments win_count' do
        create(:game, mode: 'tournament', status: 'played', tournament_id: tournament.id,
                      player_left_id: participants[0], player_right_id: participants[1], winner_id: participants[0])
        expect { manage_tournament(Game.first) }.to change {
                                                      TournamentParticipant.find_by_user_id(participants[0]).win_count
                                                    }.by(1)
      end
      it 'increments by 1 for all games' do
        ...
        create(:game, mode: 'tournament', status: 'played', tournament_id: tournament.id,
                      player_left_id: participants[1], player_right_id: participants[2], winner_id: participants[2])
        expect { manage_tournament(Game.second) }.to change {
                                                       TournamentParticipant.find_by_user_id(participants[2]).win_count
                                                     }.by(1)
        create(:game, mode: 'tournament', status: 'played', tournament_id: tournament.id,
                      player_left_id: participants[2], player_right_id: participants[0], winner_id: participants[2])
        expect { manage_tournament(Game.last) }.to change {
                                                     TournamentParticipant.find_by_user_id(participants[2]).win_count
                                                   }.by(1)
      end
    end
  ...
  describe 'manage_exaequo' do
    ...
    before do
      ...
      manage_tournament(Game.second)
      create(:game, mode: 'tournament', status: 'played', tournament_id: tournament.id,
                    player_left_id: participants[2], player_right_id: participants[3], winner_id: participants[2])
      manage_tournament(Game.third)
      create(:game, mode: 'tournament', status: 'played', tournament_id: tournament.id,
                    player_left_id: participants[3], player_right_id: participants[0], winner_id: participants[3])
      manage_tournament(Game.fourth)
      create(:game, mode: 'tournament', status: 'played', tournament_id: tournament.id,
                    player_left_id: participants[3], player_right_id: participants[1], winner_id: participants[3])
