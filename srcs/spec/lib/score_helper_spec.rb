...
RSpec.describe GamePointGiver do
  ...
  let!(:ladder_game) do
    create(:game, player_left: tom, player_right: alan, winner: tom, status: 'played', mode: 'ladder')
  end
  let!(:ladder_game_2) do
    create(:game, player_left: tom, player_right: alan, winner: alan, status: 'played', mode: 'ladder')
  end
  ...
  context 'Tournament' do
    ...
    let(:game) do
      create(:game, player_left: tom, player_right: alan, winner: tom, status: 'played', mode: 'tournament',
  ...
  context 'User has a guild' do
    ...
    context 'At war' do
      before do
        create(:war, from: nos, on: bang, war_start: DateTime.now, war_end: DateTime.new(2022), prize: 1000, opened: true,
                     from_score: 0, on_score: 0)
      end
      ...
      context 'war_time duel' do
        it 'gives points to war on_score' do
          war_time = WarTime.create(day: Date.today.strftime('%A'), start_hour: 8, end_hour: 23, war_id: War.first.id,
                                    time_to_answer: 10, max_unanswered: 2)
          war_game = create(:game, player_left: tom, player_right: alan, winner: tom, status: 'played', mode: 'war',
                                   war_time_id: war_time.id)
